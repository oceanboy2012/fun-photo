...
def move_temp_photos(high_session_path, max_photo_count, temporary_path):
	...
	count = len(temp_list)  #this function counts the number of photos in the temp folder then moves them to 
...
def change_photo_num(high_session_path, digits, temporary_path):    #pass highest session path variable to session
                                               # folder based on folder session pathway
	global glob_copy_path    #global variable
	if not os.listdir(temporary_path): # if no files in temporary_path folder

		if not os.listdir(high_session_path):   #if there are no files(photos) in folder path, make photo 00001
			glob_copy_path = temporary_path + 'session' + high_session_path[-6:-1] + '_00001.jpg' 
							#set glob variable to temp path folder	          
		...
		else:      # if there are files in highest session path
			highest_photo = sorted(os.listdir(high_session_path), reverse=False)[-1] 
                                 #gives us session0000?_0000highest
			next_photo_num = str(int(highest_photo[-9:-4]) + 1).zfill(digits)   #gives us just the photo number,
                                                # makes integer, adds 1 to make next photo number,
                                                 # makes string again
			...
			return temporary_path + 'session' + high_session_path[-6:-1] + '_' + next_photo_num + '.jpg'
		         	#returns path of session folder with session0000?_0000nextnumber.jpg
	else:            # if there are files in the temporary path: make next photo number 
		highest_photo = sorted(os.listdir(temporary_path), reverse=False)[-1] #gives us session0000?_0000highest
		next_photo_num = str(int(highest_photo[-9:-4]) + 1).zfill(digits)   #gives us just the photo number,
                                                # makes integer, adds 1 to make next photo number,
                                                 # makes string again
		glob_copy_path = temporary_path + 'session' + high_session_path[-6:-1] + '_' + next_photo_num + '.jpg'
...
subprocess.Popen(['chromium-browser', '--kiosk','/home/pi/photobooth/down_count.html'], #open browser in subprocess
	stdout=open('/dev/null', 'w'),
	stderr=open ('logfile.log', 'a'))
#os.popen( 'chromium-browser --kiosk /home/pi/photobooth/down_count.html') #start web browser countdown

while temp_x <= pics_per_group:

	time.sleep(6.60)
	os.popen( 'fswebcam -b -q -r 1280x960 -d ' + glob.glob('/dev/video*')[0] + ' ' + change_photo_num(high_path,5,temp_path))
	time.sleep(2.40)


	temp_x = temp_x + 1

subprocess.Popen(['chromium-browser', '--kiosk','/home/pi/photobooth/processing_delay.html'], #open browser in subpross
	stdout=open('/dev/null', 'w'),
	stderr=open ('logfile.log', 'a'))

copy_temp(temp_path,1) # takes a folder path and lists contents then copies each starting with 1.jpg ... up
time.sleep(2.6)
subprocess.Popen(['chromium-browser', '--kiosk','/home/pi/photobooth/simple_slideshow.html'], #open browser in subpross
	stdout=open('/dev/null', 'w'),
	stderr=open ('logfile.log', 'a'))
